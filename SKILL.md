...
4. 按 POL 分组弹出 Outlook 新建邮件草稿（TO = LSP 邮箱），POC 审核后手动发送
5. 运行结束后自动发送报告邮件至 miaoyua@amazon.com（含日志 + JSON 附件）
...
| 工具文件夹 | 将 `PushCLP\` 整个文件夹放到 `Desktop\`（含 PushCLP.exe + config.yaml + data/ 文件夹）|

> ✅ 无需安装 Python，PushCLP.exe 已内置运行时，开箱即用。
> ✅ 无需修改任何配置，config.yaml 已含所有默认值。

### data/ 目录内容（工具自带，无需手动准备）

| 文件 | 说明 |
|------|------|
| `MSPP List Review 2026May.xlsx` | MSPP Shipper 列表（每月需更新时覆盖此文件）|
| `LSP邮箱.xlsx` | LSP 邮箱对应表 |
| `Blurb_CLP.txt` | 邮件正文模板 |
...
步骤1 — 运行 PushCLP.exe
  用 Bash 执行：
    powershell.exe -Command "& \"$env:USERPROFILE\Desktop\PushCLP\PushCLP.exe\""
  等待进程完成（最多 300 秒）。若报错输出完整错误信息并继续步骤2。

步骤2 — 读取结果
  读取文件：./data/run_result.json
  解析字段：
    - stats：运行统计（total_orders/mspp_orders/target_orders/step_b_pass/step_c_pass/step_d_pass/final_pushed）
    - clp_items：最终推送的 AL0 列表（每条含 al0/pol/pod/etd/clp_cutoff/all_ready_cut）
    - match_trace：每条目标 AL0 逐步追踪（step_b/step_c/step_d 及详情）
...
步骤3 — 展示本次摘要
  在本线程输出：
    [统计] 目标订单={target_orders} | B通过={step_b_pass} | C通过={step_c_pass} | D通过={step_d_pass} | 最终推送={final_pushed}

    [CLP 清单] N 条（final_pushed 为 0 时输出"本次无需推送"）
    | AL0 | POL | POD | ETD | CLP Cutoff | All Ready Cut |
    （逐行列出 clp_items）
...
| 现象 | 原因 | 处理 |
|------|------|------|
| CLP 清单 0 条 | 本次无满足条件的订单 | 正常，报告邮件仍会发送 |
...
| 报告邮件未收到 | Outlook 未运行 | 先启动 Outlook 再运行 |
